...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function main() {
        ...
        const CONSTANTS = { STORAGE_KEY: 'multiplicationStarChartLeaderboard_offline', classes: { '2A': 25, '2B': 25, '2C': 25, '2D': 25, '2E': 25, '老師': ["陳子殷老師", "陳綺雯老師", "陳懿文老師", "楊靖霖老師", "陳慧淇老師"] } };
        ...
        const functions = {
            ...
            bindEvents() {
                ...
                ELEMENTS.viewLeaderboardBtn.addEventListener('click', () => {
                    functions.displayLeaderboard(ELEMENTS.leaderboardMainContainer);
            ...
            showScreen(name, mode) {
                ...
                if (c && mode !== 'instant') {
                    // 使用 display: none 來確保舊畫面完全消失，避免潛在的交互問題
                ...
                // 使用一個微小的延遲來確保 display 屬性先生效，然後再觸發透明度動畫
                setTimeout(() => {
            ...
            endGame() { clearInterval(state.timerId); ELEMENTS.timerDisplay.classList.remove('timer-warning'); if (state.score > 0) functions.saveToLeaderboard(state.currentUser.name, state.score); const rank = functions.getPlayerRank(state.currentUser.name); let rankMessage = ""; if (rank > 0 && rank <=10) { rankMessage = (rank === 1) ? "太棒了！你打破了最高紀錄，榮登榜首！👑" : `恭喜！你目前的排名是：第 ${rank} 名 🎉`; } else if (state.score > 0) { rankMessage = `你的準確排名是：第 ${rank} 名。加油！`; } ELEMENTS.finalScoreText.textContent = `${state.currentUser.name}，你獲得了 ${state.score} 顆星星！`; ELEMENTS.rankInfoText.textContent = rankMessage; functions.displayLeaderboard(ELEMENTS.leaderboardContainer, state.currentUser.name); functions.showScreen('end'); },
            getPlayerRank(playerName) { const board = functions.getFullLeaderboard(); const playerIndex = board.findIndex(p => p.name === playerName); return playerIndex !== -1 ? playerIndex + 1 : -1; },
            getFullLeaderboard() { const data = localStorage.getItem(CONSTANTS.STORAGE_KEY) || '[]'; const board = JSON.parse(data); return board.sort((a, b) => b.score - a.score); },
            getLeaderboard() { return functions.getFullLeaderboard().slice(0, 10); },
            saveToLeaderboard(playerName, playerScore) { let board = functions.getFullLeaderboard(); const i = board.findIndex(p => p.name === playerName); if (i > -1) { if (playerScore > board[i].score) { board[i].score = playerScore; } } else { board.push({ name: playerName, score: playerScore }); } localStorage.setItem(CONSTANTS.STORAGE_KEY, JSON.stringify(board)); },
            displayLeaderboard(container, currentPlayerName = null) { container.innerHTML = '<h2>排行榜</h2>'; const board = functions.getLeaderboard(); if (board.length === 0) { container.innerHTML += '<p>還沒有人上榜！</p>'; return; } const list = document.createElement('ol'); list.className = 'leaderboard-list'; board.forEach((p, i) => { const item = document.createElement('li'); item.className = 'leaderboard-item'; if (p.name === currentPlayerName) item.classList.add('current-player'); const nS = document.createElement('span'); nS.textContent = `${i + 1}. ${p.name}`; const sS = document.createElement('span'); sS.textContent = `⭐ ${p.score}`; item.appendChild(nS); item.appendChild(sS); list.appendChild(item); }); container.appendChild(list); },
